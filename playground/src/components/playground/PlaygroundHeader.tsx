...
import { WakeWordIndicator } from "@/components/playground/WakeWordIndicator";
import { WakeWordState } from "@/hooks/useWakeWord";
import { ConnectionQualityIndicator, useLocalParticipant } from "@livekit/components-react";
...
export const PlaygroundHeader = ({
  ...
  // Get local participant for connection quality indicator
  // This hook is safe to call even when not connected - it returns undefined
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="text-xs lg:text-base lg:font-semibold text-white">
          {title}
        </div>
        {/* Wake Word Indicator - show when connected */}
        {connectionState === ConnectionState.Connected && wakeWordState && wakeWordState !== "disabled" && (
          <WakeWordIndicator
            state={wakeWordState}
      ...
      <div className="flex justify-end items-center gap-2">
        ...
        {connectionState === ConnectionState.Connected && localParticipant && (
          <div className="flex items-center text-white">
            <ConnectionQualityIndicator participant={localParticipant} />
          </div>
        )}
        {connectionState === ConnectionState.Connected && onSettingsClick && (
          <button
